...
CLEAR_BEFORE_RUN = True  # if True, output window is cleared before each run
...
class LogWindow(rur_editor):
    def __init__(self, parent):
        rur_editor.__init__(self, parent)
        self.SetKeyWords(0, " ")
    ...
    def setHighlightMode(self, mode=True):
        self.highlightMode = mode
        if mode:
            self._n = 0
            self.highlight(self._n)
            self.Refresh()
            self.GotoLine(self._n)
        else:
            self.remove_highlight()

    def Undo(self):
        self.SetReadOnly(False)
        rur_editor.Undo(self)
        self.SetReadOnly(True)
    ...
    def InsertText(self, pos, text):
        self.SetReadOnly(False)
        self.BeginUndoAction()
        rur_editor.InsertText(self, pos, text)
        self.EndUndoAction()
        self.SetReadOnly(True)

    def ClearAll(self):
        self.SetReadOnly(False)
        rur_editor.ClearAll(self)
        self.EmptyUndoBuffer()
        self.SetReadOnly(True)

    def write(self, text):
        '''required method for sys.stdout and sys.stderr handling'''
        if not self.highlightMode:
            self.AddText(text)
