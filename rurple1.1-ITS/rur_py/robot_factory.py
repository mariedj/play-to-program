...
class Robot_brain1(object):
    ...
    def _undo(self):
        '''Undo the last robot action'''
        if self._undo_history:
            r = self._undo_history.pop()
            r()

    def _stay_put(self):
        '''Add a null action to the undo history'''
        def r():
            pass
        self._undo_history.append(r)
...
class Used_robot(Robot_brain1):
    ...
    def move(self):
        Robot_brain1.move(self)
        base_r = self._undo_history.pop()
        def r():
            base_r()
            self.line_trace.pop()
        self._undo_history.append(r)
