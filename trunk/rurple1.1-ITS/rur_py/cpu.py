        ...
        if event == "line":
            ...
            if 'robot2' in self.robot_dict:
                ...
                while txt.startswith('IN'):
                    ...
                    if txt.count('Move'):
                        self.move('robot2')
                    elif txt.count('Turn Left'):
                        self.turn_left('robot2')
                    elif txt.count('Turn Right'):
                        self.turn_right('robot2')
                    txt, num = self.parent.outputWindow.GetCurLine()
    ...
    def execute_program(self):
        ...
        try:
            ...
            if self.parent.outputWindow.highlightMode:
                self.parent.outputWindow.setHighlightMode(True)
...
class UsedRobot(object):
    ...
    def drop_beeper(self):  # new name
        self.program.put_beeper(self.name)
    def put_beeper(self):  # old name kept for compatibility...
        self.program.put_beeper(self.name)
    def grab_beeper(self):  # new name
        self.program.pick_beeper(self.name)
    def pick_beeper(self):  # old name kept for compatibility...
        self.program.pick_beeper(self.name)
